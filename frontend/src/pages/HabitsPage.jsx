...
const DEFAULT_FORM = { title: '', description: '', icon: '✅', color: '#6366f1', frequency: 'daily', target_days: 7 };
...
export default function HabitsPage() {
  ...
  const openEdit = (h) => { setEditing(h); setForm({ title: h.title, description: h.description || '', icon: h.icon, color: h.color, frequency: h.frequency, target_days: h.target_days }); setShowModal(true); };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold text-ink-900">Os meus hábitos</h1>
          <p className="text-ink-400 text-sm font-body mt-1">{habits.length} hábito{habits.length !== 1 ? 's' : ''} activo{habits.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 gap-4">
          {[1,2,3,4].map(i => <div key={i} className="card p-6 animate-pulse"><div className="h-5 bg-ink-100 rounded w-2/3 mb-3"/><div className="h-4 bg-ink-100 rounded w-1/2"/></div>)}
        </div>
      ) : habits.length === 0 ? (
        <div className="card p-16 text-center">
          <div className="text-6xl mb-5">🌱</div>
          <h2 className="font-display text-2xl font-semibold text-ink-800 mb-2">Nenhum hábito ainda</h2>
          <p className="text-ink-400 font-body mb-8">Crie o seu primeiro hábito e comece a sua jornada</p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {habits.map((habit, i) => (
            <div key={habit.id} className="card p-5 hover:shadow-md transition-all duration-300 animate-slide-up group"
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-body font-semibold text-ink-900 truncate">{habit.title}</h3>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                      <button onClick={() => openEdit(habit)}
                        className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-ink-100 text-ink-400 hover:text-ink-700 transition-colors text-sm">
                        ✏️
                      </button>
                      <button onClick={() => setConfirmDelete(habit.id)}
                        className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 text-ink-400 hover:text-red-600 transition-colors text-sm">
                        🗑️
                      </button>
                    </div>
                  </div>
                  {habit.description && (
                    <p className="text-ink-400 text-xs font-body mt-1 line-clamp-2">{habit.description}</p>
                  )}
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    {habit.completed_today && (
                      <span className="badge bg-sage-500/20 text-sage-700">✓ Hoje</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 h-1 rounded-full" style={{ backgroundColor: `${habit.color}40` }}>
                <div className="h-full rounded-full transition-all duration-700"
                     style={{ width: `${Math.min((habit.total_completions / Math.max(habit.target_days, 1)) * 100, 100)}%`, backgroundColor: habit.color }} />
              </div>
            </div>
          ))}
        </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative card p-8 w-full max-w-sm animate-slide-up text-center">
            ...
            <h3 className="font-display text-xl font-bold text-ink-900 mb-2">Eliminar hábito</h3>
            <p className="text-ink-500 text-sm font-body mb-6">Esta acção não pode ser desfeita.</p>
          ...
          <div className="relative card p-8 w-full max-w-lg animate-slide-up max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={submit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="label">Meta (dias)</label>
                  ...
                  {ICONS.map(icon => (
                    <button type="button" key={icon} onClick={() => setForm(p => ({ ...p, icon }))}
                      className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl transition-all ${
                        form.icon === icon ? 'bg-ink-900 shadow-md scale-110' : 'bg-ink-100 hover:bg-ink-200'
                      }`}>
                      {icon}
                    </button>
                  ))}
                ...
                <div className="flex gap-3 flex-wrap">
                  {COLORS.map(color => (
                    <button type="button" key={color} onClick={() => setForm(p => ({ ...p, color }))}
                      ...
                      className={`w-8 h-8 rounded-full transition-all ${form.color === color ? 'ring-2 ring-offset-2 ring-ink-700 scale-110' : 'hover:scale-105'}`} />
                  ))}
                </div>
              </div>

              {/* Preview */}
              <div className="p-4 rounded-xl border border-ink-100 bg-ink-50 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
                     style={{ backgroundColor: `${form.color}22` }}>{form.icon}</div>
                <div>
                  <p className="font-body font-medium text-ink-900">{form.title || 'Pré-visualização'}</p>
                  <p className="text-xs text-ink-400 font-body">{form.frequency === 'daily' ? 'Diário' : 'Semanal'}</p>
                </div>
